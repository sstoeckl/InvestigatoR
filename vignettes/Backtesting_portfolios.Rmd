...
#' Backtest Portfolios Given Return Predictions
#'
#' This function is designed to backtest portfolios based on provided return predictions. It allows users to configure multiple models and weight configurations to evaluate portfolio performance.
#'
#' @param return_prediction_object A returnPrediction S3 object containing predicted returns and associated errors.
#' @param portfolio_object A portfolio S3 object to which the backtested portfolio will be appended. If NULL, a new portfolio object will be created.
#' @param pf_config A list of configurations for the portfolio, specifying models and weight configurations.
#' @param append Boolean indicating whether to append the backtested portfolio to the existing portfolio object. Defaults to FALSE.
#'
#' @return A portfolio object that can be further processed using summary or plot functions.
#'
...
#'                  min_weight = -1, max_weight = 1, b = 1)))
#' 
#' return_prediction_object <- create_return_prediction_object() # Replace with actual function to create returnPrediction object
#' portfolio_object <- create_portfolio_object() # Replace with actual function to create portfolio object
#' result <- backtesting_portfolios(return_prediction_object, portfolio_object, pf_config)
#' }
backtesting_portfolios <- function(return_prediction_object, portfolio_object = NULL, pf_config, append = FALSE) {
  ...
  for (i in 2:length(pf_config)) {
    ...
    for (j in 2:length(pf_config[[model_name]])){
      ...
      new_weights <- training_function(specific_predictions, specific_errors, weight_config)
