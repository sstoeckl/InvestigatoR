...
config_keras_increase_leverage <- list(
  ...
  loss = list(
    ...
    lambda = 0.1, 
    leverage = 1.0, 
    eta = 0.7  # Stronger leverage penalty
  ),
  optimizer = list(name = "optimizer_adam", learning_rate = 0.001),
  metrics = list(leverage_metric()),
  callbacks = list(
    callback_early_stopping(monitor = "loss", min_delta = 0.001, patience = 3)
  ),
  epochs = 100,  # More epochs with early stopping
...
config_keras_increase_turnover <- list(
  layers = list(
    list(type = "dense", units = 32, activation = "relu"),
    list(type = "dense", units = 16, activation = "relu"),
    list(type = "dense", units = 1, activation = activation_box_sigmoid(min_weight = 0, max_weight = 0.20))
  ),
  loss = list(
    name = "sharpe_ratio_loss", 
    transaction_costs = 0.01,  # Increase transaction costs to penalize high turnover
    delta = 0.1, 
    lambda = 0.1, 
    leverage = 1.0, 
    eta = 0.1
...
weights_summary <- summary(keras_portfolios)
...
summary(keras_portfolios, type = "table.Distributions")
```
...
# Verify that benchmark weights sum to 1 for each date
benchmark_sum <- data_ml %>%
  group_by(date) %>%
  summarize(total_benchmark = sum(benchmark))
...
config_information_ratio_active <- list(
  layers = list(
    list(type = "dense", units = 32, activation = "relu"),
    list(type = "dense", units = 16, activation = "relu"),
    list(type = "dense", units = 1, activation = activation_box_sigmoid(min_weight = -0.2, max_weight = 0.2))  # Bound delta_w
...
config_information_ratio_regression <- list(
  layers = list(
    list(type = "dense", units = 32, activation = "relu"),
    list(type = "dense", units = 16, activation = "relu"),
    list(type = "dense", units = 1, activation = activation_box_sigmoid(min_weight = -0.2, max_weight = 0.2))  # Bound delta_w
...
# Summarize the portfolio results
weights_summary_advanced <- summary(advanced_keras_portfolios)
print(weights_summary_advanced)

# Visualize the portfolio performance
plot(advanced_keras_portfolios)

# Additional analysis using PerformanceAnalytics
summary(advanced_keras_portfolios, type = "table.AnnualizedReturns")
summary(advanced_keras_portfolios, type = "table.Distributions")
```
